...
lazy val root = (project in file("."))
  .aggregate(catalogService, orderService)
  .settings(
...
lazy val catalogService = (project in file("catalog-service"))
  ...
  .settings(
    ...
    Compile / run / mainClass := Some("com.endsoullab.CatalogApplication"),
...
lazy val orderService = (project in file("order-service"))
  ...
  .settings(
    ...
    // 자동 생성될 스칼라 객체의 패키지명
    buildInfoPackage := "com.endsoullab.order",
    ...
    Compile / run / mainClass := Some("com.endsoullab.order.OrderApplication"),
    Compile / run / fork := true,
    Test / fork := true,
    // forked 테스트 JVM의 작업 디렉터리를 catalog-service/가 아닌 저장소 루트로 맞춰
    // 루트의 .scalafmt.conf 등 상대 경로 리소스를 정상적으로 찾도록 함
